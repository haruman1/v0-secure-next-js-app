...
export default function PenerbitanPage() {
  ...
  async function handleUploadResultDocument(appId: string) {
    ...
    if (!file) {
      Swal.fire({ icon: 'warning', text: t('pages.penerbitan.pilihFile') });
      return;
    }

    // Validasi 4 Jam
    const app = applications.find((a) => a.id === appId);
    if (app && app.tanggalPerjalanan) {
      const travelDateStr = app.tanggalPerjalanan.slice(0, 10);
      const travelDateTime = new Date(
        `${travelDateStr}T${app.jamPerjalanan || '00:00'}:00`,
      );
      const now = new Date();
      ...
      if (travelDateTime.getTime() - now.getTime() < 4 * 60 * 60 * 1000) {
        Swal.fire({
          ...
          text:
            language === 'id'
              ? 'Penerbitan dokumen ditolak karena waktu keberangkatan kurang dari 4 jam dari sekarang.'
              : 'Document publication rejected because departure time is less than 4 hours from now.',
        });
        return;
      }
    }
    ...
    // --- COMPRESSION LOGIC ---
    let finalFile = file;
    const validationResult = validateFileSize(file, 10);
    const isValid =
      typeof validationResult === 'object' && validationResult !== null
        ? (validationResult as Record<string, any>).isValid
        : validationResult;
    const error =
      typeof validationResult === 'object' && validationResult !== null
        ? (validationResult as Record<string, any>).error
        : undefined;
    if (!isValid) {
      Swal.fire({ icon: 'error', text: error || t('messages.maxSize') });
      return;
    }
    ...
    try {
      ...
      if (!uploadRes.ok || !uploadResult?.data?.path) {
        throw new Error(
          uploadResult?.error || t('pages.penerbitan.gagalUpload'),
        );
      }
      ...
      if (!saveRes.ok) throw new Error(t('pages.penerbitan.gagal'));

      Swal.fire({
        icon: 'success',
        title: 'Sukses',
        text: t('pages.penerbitan.sukses'),
      });
      setSelectedFiles((prev) => ({ ...prev, [appId]: null }));
      setApplications((prev) =>
        prev.map((app) =>
          app.id === appId
            ? { ...app, dokumenPenerbitan: uploadResult.data.path }
            : app,
        ),
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : t('pages.penerbitan.gagal');
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 max-w-6xl mx-auto bg-slate-50/50 min-h-screen text-slate-800">
      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
            {t('pages.penerbitan.title')}
          </h1>
          <p className="text-slate-500 text-base md:text-lg mt-2 max-w-2xl">
            {isAdmin
              ? t('pages.penerbitan.descAdmin')
              : t('pages.penerbitan.descUser')}
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl md:text-2xl font-bold text-slate-900">
                {isAdmin
                  ? t('pages.penerbitan.kelolaAdmin')
                  : t('pages.penerbitan.kelolaUser')}
              </CardTitle>
              <CardDescription className="text-base text-slate-500 mt-1 font-medium">
                {isAdmin
                  ? `${appsToProcess.length} ${t('pages.penerbitan.validDitemukan')}`
                  : `${userPublishedDocuments.length} ${t('pages.penerbitan.dokumenTersedia')}`}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-6 md:p-10">
          {loading ? (
            ...
            appsToProcess.length === 0 ? (
              <div className="py-20 flex flex-col items-center text-slate-500 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                ...
                <p className="text-lg font-medium">
                  {t('pages.penerbitan.kosongAdmin')}
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                {appsToProcess.map((app) => {
                  ...
                  return (
                    <div
                      key={app.id}
                      className="rounded-2xl border border-slate-200 p-6 md:p-8 flex flex-col lg:flex-row gap-8 bg-white shadow-sm hover:shadow-md transition-shadow"
                    >
                      {/* INFO PASIEN (KIRI) */}
                      <div className="flex-1 space-y-5">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-none px-4 py-1.5 text-sm font-semibold rounded-full shadow-sm">
                            {t('pages.penerbitan.disetujui')}
                          </Badge>
                        </div>

                        <p className="text-lg text-slate-700 font-medium flex items-center gap-2">
                          {app.namaMaskapai || '-'}{' '}
                          <span className="text-slate-300">•</span> Waktu
                          Penerbangan{' '}
                          {formatTanggalID(app.tanggalPerjalanan, language) ||
                            '-'}{' '}
                        ...
                        <div className="flex flex-wrap gap-3 items-center pt-2">
                          <span className="flex items-center gap-1.5 text-sm font-medium text-slate-500 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                            ...
                            {t('pages.penerbitan.dibuat')}{' '}
                            {formatTanggalID(app.created_at, language)}{' '}
                            {formatTime(app.created_at, language)}
                          </span>
                      ...
                      <div className="lg:w-[400px] flex flex-col justify-center space-y-4">
                        ...
                        {documentUrl ? (
                          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm text-blue-600 font-semibold">
                                  {t('pages.penerbitan.fileTerlampir')}
                                </p>
                                <p
                                  className="text-base text-slate-800 font-medium truncate"
                            ...
                            <div className="flex gap-2">
                              <Button
                                variant="outline"
                                className="flex-1 bg-white hover:bg-blue-50 border-blue-200 text-blue-700 h-10"
                                onClick={() => openServerPreview(documentUrl)}
                              >
                                {t('pages.penerbitan.reviewDokumen')}
                              </Button>
                              <Button
                              ...
                              >
                                <a
                                  ...
                                  download
                                >
                                  {t('pages.penerbitan.downloadDokumen')}
                                </a>
                              </Button>
                            </div>
                          </div>
                          ...
                          <p className="text-base text-slate-500 font-medium">
                            {t('pages.penerbitan.belumAdaDokumen')}
                          </p>
                        )}

                        {/* DRAG & DROP CUSTOM UPLOAD ZONE */}
                        <div id="tour-pene-upload" className="relative">
                          <input
                            type="file"
                            id={`file-${app.id}`}
                            ...
                            disabled={
                              !!isUploading
                            } /* Memastikan selalu terbaca sebagai boolean oleh TypeScript */
                            className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                          />
                          <div
                            className={`border-2 border-dashed rounded-xl p-6 text-center transition-all ${selectedFile ? 'border-blue-400 bg-blue-50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100 hover:border-slate-400'}`}
                          >
                            <UploadCloud
                              className={`w-10 h-10 mx-auto mb-3 ${selectedFile ? 'text-blue-500' : 'text-slate-400'}`}
                            />
                            <p
                              className={`text-base font-semibold truncate ${selectedFile ? 'text-blue-700 px-2' : 'text-slate-600'}`}
                            >
                              {selectedFile
                                ? selectedFile.name
                                : t('pages.penerbitan.pilihFile')}
                            </p>
                            {!selectedFile && (
                              <p className="text-sm text-slate-400 mt-1">
                                {t('pages.penerbitan.formatAlert')}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        {selectedFile ? (
                          <div className="flex gap-3">
                            ...
                            >
                              {isUploading
                                ? t('pages.penerbitan.menyimpan')
                                : documentUrl
                                  ? t('pages.penerbitan.updateSuratIzin')
                                  : t('pages.penerbitan.uploadSuratIzin')}
                            </Button>
                          </div>
                        ) : (
                          <Button
            ...
            <div className="py-20 flex flex-col items-center text-slate-500 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
              ...
              <p className="text-lg font-medium">{t('pages.penerbitan.kosongUser')}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userPublishedDocuments.map(({ app, documentUrl }) => (
                <div
                  key={app.id}
                  className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between h-full group"
                >
                  ...
                  <div className="flex gap-3 mt-6 pt-6 border-t border-slate-100">
                      <Button
                        variant="outline"
                        className="flex-1 text-base h-11 border-slate-200 hover:bg-slate-50 rounded-xl"
                        onClick={() => openServerPreview(documentUrl)}
                      >
                        {t('common.view')}
                      </Button>
                      <Button
                        className="flex-1 text-base h-11 bg-slate-900 hover:bg-slate-800 text-white rounded-xl"
                        asChild
                      >
                        <a
                          href={documentUrl}
      ...
      >
        <DialogContent className="max-w-5xl w-[90vw] p-0 rounded-2xl overflow-hidden border-none shadow-2xl">
          <DialogHeader className="p-6 md:p-8 bg-slate-900 text-white border-b border-slate-800">
            ...
            <DialogDescription className="text-base text-slate-400 mt-1">
              {t('pages.penerbitan.descPreview')}
            </DialogDescription>
          </DialogHeader>
