...
export async function POST(
  ...
  try {
    ...
    const evacuationResults = await query(
      'SELECT id, status FROM air_medical_evacuation WHERE id = ?',
      [id],
    );
    ...
    const evacuation = evacuationResults[0] as { id: string; status: string };

    if (evacuation.status !== 'valid') {
      return NextResponse.json(
        { error: 'Dokumen penerbitan hanya untuk permohonan yang sudah disetujui' },
        { status: 400 }
      );
    }
